...
dgp.lambdas ...
    = [0.2 0.5 0.8 0.95];        % values of AR(2) repeated roots
...
sim.num_workers ...
    = 8;                                  % No. of parallel workers 
                                          % (=0: run serial)

% Reporting
results_filename ...
    = sprintf('%s%s', 'sim_ar2_equalroot_', dgp.type);  % File name for storing results                                                                
...
settings.p ...
         = 2;                        % Lag length used for estimation 
...
aux1   = repmat(dgp.lambdas,size(dgp.Ts));
aux2   = repmat(dgp.Ts',size(dgp.lambdas))';
...
for i_dgp = 1:numdgp
    ...
    i_lambda = dgps(1,i_dgp);
    
    i_rho1 = 2*i_lambda;           % rho1 = 2*lambda
    
    i_rho2 = -i_lambda^2;          % rho2 = -lambda^2
...
%     for i=1:numrep
        ...
        i_Y = filter(1, [1 -i_rho1 -i_rho2], i_U); % Data series (with y_0=0)
